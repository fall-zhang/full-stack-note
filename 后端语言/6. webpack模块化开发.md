...
module.exports = {
  ...
  // 告诉webpack 将从那个文件开始构建，这个文件将作为 webpack 关系依赖图的起点
  output:{
...
// 也可以更改webpack 后面的参数，webpack-dev-server --open --host 127.0.0.1 --port 8888
...
```js
"scripts":{
  "watch":"webpack --watch" // 不再是通过 webpack 进行执行
}
```
...
常见值https://link.juejin.im?target=https%3A%2F%2Fwww.webpackjs.com%2Fconfiguration%2Ftarget%2F
