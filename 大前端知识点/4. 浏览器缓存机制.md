...
> Recently revised in 2021年10月23日23:46:57
...
> **强制缓存** 浏览器不会向服务器发送请求，而是直接从本地缓存中读取内容，返回200的状态码。
>
...
> **If-Modified-Since** 
>
> 再次进入该网页，浏览器再次发送请求时，会将上次的时间先写入`If-Modified-Since` ，服务器接收后，会和`Last-Modified`对比，如果`Last-Modified`大于上一次修改时间(`Last-Modified-Since`) 会显示已经修改，重新返回资源并显示 200，否则返回304，使用浏览器的缓存。
