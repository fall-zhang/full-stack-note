...
性能就是页面的表现，包括响应（Response）动画（Animation）空白时间（Idle）
...
不同的渲染方式，会导致渲染快慢，根据应用的场景，选择对应的渲染方式

- js 渲染：首次渲染慢，交互渲染更快
- html+css 渲染：首次渲染更快，交互渲染更慢
...
- **减少 DOM 节点**：DOM 节点太多影响页面的渲染，尽量使用更少的 DOM 节点实现功能
...
- 避免 CSS 表达式：CSS 表达式的执行需跳出 CSS 树的渲染（CSS expression 关键字，基本没人使用了）
...
- **不滥用 float**：float 在渲染时计算量比较大，尽量不去使用，用 flex 代替
