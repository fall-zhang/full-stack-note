...
| `$route.params`   | 一个 key/value 对象，包含了 动态片段 和 全匹配片段，<br/>如果没有路由参数，就是一个空对象。 |
| `$route.query`    | 一个 key/value 对象，表示 URL 查询参数。<br/>例如，对于路径 /foo?user=1，则有 `$route.query.user == 1`，<br/>如果没有查询参数，则是个空对象。 |
