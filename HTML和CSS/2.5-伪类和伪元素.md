...
> Recently revised in 27 Nov 2025
...
```html
<p>
  <b>Mars</b> is one of the most Earth-like planets.
</p>
...
该选择器和 `:is()` 差不多，区别是 `:where()` 选择器的优先级为 0
...
/* 上方的内容和下方的内容等价（where 中的选择器优先级为 0） */
header p:hover,
main p:hover,
footer p:hover {
  color: red;
...
Baseline 2023，之后的浏览器基本可用

表明当前选择器，在拥有某些兄弟选择器时，应用效果
