...
> Create by **fall** on 27 Dec 2021<br/>
...
Fetch API 是近年来被提及将要取代 XHR 的技术新标准。
Fetch 并不是 XHR 的升级版本，而是从一个全新的角度来思考的一种设计。Fetch 是基于 Promise 语法结构，而且它的设计足够低阶，这表示它可以在实际需求中进行更多的弹性设计。对比**XHR**所提供的能力来说，Fetch 已经足够取代 XHR，并且提供了更多拓展的可能性。
...
- - cors：允许跨域，要求响应中 Acess-Control-Allow-Origin 这样的头部表示允许跨域。 		
  - no-cors：只允许一组有限的 HTTP 请求头，以及简单的 HEAD，GET，POST 方法。 		
...
fetch(url, {
  ...
  credentials: 'same-origin', // include, *same-origin, *omit  表示是否发送 cookie，same-origin 仅在同源时发送
  headers: { // 请求头信息，可以是对象或 Headers 实例化的对象
    'user-agent': 'Mozilla/4.0 MDN Example',
    'content-type': 'application/json'
  },
  method: 'POST', // *GET, POST, PUT, DELETE, etc.
  mode: 'cors', // no-cors, *cors, same-origin
  redirect: 'follow', // manual, *follow, error 表示发生重定向时，有三个选项，手动，跟随，报错
...
fetch 的第二个参数中的 body 属性，表示需要发送的信息内容，可以为以下任一类型
...
const query = searchParams.toString() // name=%E5%88%98
// 使用 ? 进行拼接，服务器只需照常接受数据即可
fetch('http://localhost:3000/books?'+query,{
  method:"get"
})

// 使用 '/' 进行拼接，即 restful 形式，服务器需要更改数据的接收方式
fetch('http://www.cctv.com/books/3',{
  method:'get'
})
// 服务器端代码
app.get('/books/:id',(req,res)=>{
  res.send('restful 形式的 url 请求传递参数'+req.params.id)
})
```
...
fetch('http://localhost:2333',{
  ...
  body:formData // 或者直接传递上面的 formData
}).then(resopnse=>{
  return resopnse.json()
}).then(res=>{
  console.log(resopnse)
...
searchParams.toString() // 'dkd=df&name=lao'
```
...
> It’s been available across browsers since ⁨2019年3月⁩.
...
> 详情请见 [axios 的介绍](../npm 包/axios.md)

既可以生成一个新的请求相关配置，也可以在 axios 的基础上进行配置，这里选择后者。

首先需要配置 axios

```js
axios.defaults.timeout = 5000 // 响应超时时间
axios.defaults.baseURL = 'http://localhost:3000' // 默认基准地址，之后的请求会自动添加上 'http://localhost:3000'
...
`var xhr = new XMLHttpRequest()` IE8 以下不兼容
