...
Mongo 不会在磁盘保存视图内容，视图内容将根据查询视图时，按需计算得出。一般用于下面的场景

- 过滤一些信息，排除一些字段信息
- 添加计算得出的字段信息
- 连接两个不同的集合，库存集合以及订单集合。

mongo 提供两种，和

- 标准视图：读取视图时计算得出，并且不会在磁盘中存储。
- 按需物化视图：存储在磁盘上并从磁盘读取，使用 `$merge`，`$out` 阶段更新保存的数据

使用视图时需要注意以下几点

- 索引：标准视图使用底层集合的索引，因此无法创建、删除、重新构建索引，也无法获取视图上的索引列表。按需物化视图可以创建索引
- 性能：因为直接存储在磁盘上，所以按需物化视图性能更好，聚合管道越复杂，聚合数据量越大，性能优势越明显
- 视图只读，写入操作会返回错误
...
| 作者     | 文章名称                                                     |
| -------- | ------------------------------------------------------------ |
| 官方文档 | [MongoDB](https://www.mongodb.com/zh-cn/docs/manual/)        |
