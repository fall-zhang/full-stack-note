...
安装： 从淘宝镜像上[下载 git](https://npm.taobao.org/mirrors/git-for-windows/)
...
* 按 i 键切换到编辑状态，输入规则，例如 node\_modules/，然后按 Esc 键退出编辑，输入 `:wq` 保存退出。

### 生成密钥

登录过程中可能需要用到秘钥，作为服务器端和本地唯一识别认证码，一般也可以使用 git 直接进行登录
...
Git 子模块允许你将一个 Git 仓库作为另一个 Git 仓库的子目录，同时保持各自的**提交历史独立**。

```bash
git submodule add <子项目仓库URL> <本地路径>
# 克隆包含子模块的项目后，需要初始化子模块
git submodule init
# 更新子模块到指定提交
...
Git 子树允许你将一个仓库作为子目录合并到另一个仓库中，所有文件会成为**主项目的一部分**。
